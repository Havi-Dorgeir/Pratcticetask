...
import WaterFragmentShader from "./water_lava_shader.glsl?raw";
import WaterVertexShader from "./water_lava_vertex_shader.glsl?raw";
import { UnrealBloomPass } from "three/examples/jsm/postprocessing/UnrealBloomPass";
import { EffectComposer } from "three/examples/jsm/postprocessing/EffectComposer";
import { RenderPass, ShaderPass } from "three/examples/jsm/Addons.js";


const BLOOM_LAYER = 1; // объекты на этом слое будут светиться
...
const waterMaterial = new THREE.ShaderMaterial({
    ...
    uniforms: {
    uTime:  { value: 0.0 },
    uDeep:  { value: new THREE.Color('#000000') },
    uMid:   { value: new THREE.Color('#260505') },
    uCrest: { value: new THREE.Color('#9e1b14') }, 
    },
    ...
    transparent: true,
    depthWrite: false, // Часто нужно для полупрозрачной воды/эффектов, чтобы не ломать прозрачность
    depthTest: true
});
const loader = new GLTFLoader();
loader.load("/model/easy_only_hard_shader.gltf", function (glb) {
    model = glb.scene;
    scene.add(model);
    ...
    if (glb.animations && glb.animations.length > 0) {
        ...
        glb.animations.forEach((clip) => {
            ...
            if (clip.name === 'CameraScrollAnimation') {
